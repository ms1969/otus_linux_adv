...
4. Настройка симетричного роутинга
...
Теперь мы видим, что через порт идут не только пакеты echo-request, но и пакеты echo-reply, что говорит нам о симетричности трафика.


Материалы работы с коментариями, включая Vagrantfile, находяться в папке lesson32 проекта.  
