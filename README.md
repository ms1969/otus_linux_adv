...
2. DHCP сервер выдает клиенту IP адрес, в качестве опции сообщает адрес TFTP сервера и имя файла.
3. Клиент с развернутого pxeсервера с помощью протокола TFTP забирает загрузчик и описания (меню) для дальнейшей загрузки.
4. На разверунутом web сервере скачается и публикуется в виде файловой структуры образ устанавливаемой операционной системы.
5. После выбора варианта установки посредством меню, осуществляеся загрузка и установка ОС с web сервер.
