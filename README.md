...
2. DHCP сервер выдает клиенту IP адрес, в качестве опции сообщает адрес TFTP сервера и имя файла загрузки (в нашем случае "pxelinux.0").
3. 'pxeclient' выполняет загрузку с 'pxeserver' и отображает (меню), описанное в файле '/var/lib/tftpboot/pxelinux.cfg/default' 'pxeserver' для выбора варианта установки.
4. После выбора варианта установки посредством меню, осуществляеся установка ОС с web сервер.
...
```
2. Скачиваем iso образ операционной системы из Интеренета, используем другую ссылку для скачивания образа, т.к. ссылка указанная в методичке недоступна 

![Image 38](lesson28/wgetiso.png)

монтируем его, создаем каталог /iso, копируем в него файлы ОС и меняем на каталог права
