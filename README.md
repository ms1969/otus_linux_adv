...
3. После проверки настраиваем ассиметричный роутинг.

Для этого на первом маршрутизаторе увеличиваем до `1000` стоимость доставки пакетов через интерфейс `enp0s8`
![Image 323](lesson32/3.png)

После настройки запускаем пинг от интерфейса  `enp0s8` с source адресом `192.168.10.1`
```
...
После внесения данных настроек, мы видим, что маршрут до сети 192.168.20.0/30  идёт через router2, но обратный трафик от router2 идёт по другому пути.
